...
var (
	...
	global = New(6, NewMemoryStore(CollectNum, Expiration))
)
...
func Default() *CaptchaEngine {
	return global
}

func New(length int, store Store) *CaptchaEngine {
...
func (c *CaptchaEngine) New() string {
	id := randomId()
	c.Reload(id)
	return id
}
...
// Reload digits
func (c *CaptchaEngine) Reload(id string) {
	if id == "" {
		id = c.DefaultId
	}
	code := RandomDigits(c.Length)
	c.Store.Set(id, code)
}
...
func (c *CaptchaEngine) WriteImage(w io.Writer, id string, width, height int) error {
	d := c.Store.Get(id)
	if d == nil {
		return ErrNotFound
	}
...
func (c *CaptchaEngine) WriteAudio(w io.Writer, id string, lang string) error {
	d := c.Store.Get(id)
	if d == nil {
		return ErrNotFound
	}
...
func (c *CaptchaEngine) Verify(id string, digits []byte) bool {
...
func (c *CaptchaEngine) VerifyString(id string, digits string) bool {
	if digits == "" {
		return false
	}
